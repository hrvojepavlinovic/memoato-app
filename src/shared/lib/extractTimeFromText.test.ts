...
describe("extractTimeFromText", () => {
  it("extracts 24h time", () => {
    ...
    expect(out).toEqual(expect.objectContaining({ hour: 20, minute: 0, source: "20:00" }));
  });

  it("extracts 12h time with minutes", () => {
    const out = extractTimeFromText("met at 10:20 am");
    expect(out).toEqual(expect.objectContaining({ hour: 10, minute: 20, source: "10:20 am" }));
  });

  it("extracts am/pm without minutes", () => {
    const out = extractTimeFromText("football 6pm");
    expect(out).toEqual(expect.objectContaining({ hour: 18, minute: 0, source: "6pm" }));
  });
