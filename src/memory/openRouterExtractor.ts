...
const PARSER_VERSION = "openrouter-memory-extract-v0";
...
function normalizeExtraction(value: any): MemoryExtraction {
  const facts = Array.isArray(value?.facts) ? value.facts : [];
  const unknowns = Array.isArray(value?.unknowns) ? value.unknowns.filter((v: unknown) => typeof v === "string") : [];
  return {
    parser: "openrouter",
    ...
    facts: facts
      ...
      .map((fact: any) => ({
        ...
        categoryCandidates: Array.isArray(fact.categoryCandidates)
          ? fact.categoryCandidates.filter((v: unknown) => typeof v === "string").map((v: string) => v.trim()).filter(Boolean)
          : undefined,
        amount: typeof fact.amount === "number" && Number.isFinite(fact.amount) ? fact.amount : undefined,
